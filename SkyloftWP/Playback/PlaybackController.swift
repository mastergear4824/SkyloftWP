...
class PlaybackController: ObservableObject {
    ...
    func next() {
        ...
        // ✅ 현재 영상의 실제 인덱스를 다시 찾기 (videos 배열 변경에 대응)
        var actualIndex = 0
        if let current = currentVideo,
           let foundIndex = videos.firstIndex(where: { $0.id == current.id }) {
            actualIndex = foundIndex
        }
        
        var nextIdx = actualIndex + 1
        if nextIdx >= videos.count {
            nextIdx = 0
        }
        
        print("▶️ [Next] \(actualIndex) → \(nextIdx) (total: \(videos.count))")
        play(video: videos[nextIdx])
    }
    ...
    func previous() {
        ...
        var prevIdx = actualIndex - 1
        if prevIdx < 0 {
            prevIdx = videos.count - 1
        }
        
        print("◀️ [Previous] \(actualIndex) → \(prevIdx) (total: \(videos.count))")
